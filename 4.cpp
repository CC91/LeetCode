...
class Solution {
public: // http://yucoding.blogspot.com/2013/01/leetcode-question-50-median-of-two.html
    double findMedianSortedArrays(vector<int>& nums1, vector<int>& nums2) {
        int m = nums1.size(), n = nums2.size();
        int len = m+n;
        if (len%2==1) return findK(nums1.begin(), m, nums2.begin(), n, len/2+1);
    ...
    double findK(vector<int>::iterator it1, int m, vector<int>::iterator it2, int n, int k) {
        ...
        if (k==1) return min(*it1, *it2); // base case!!!
        int pa = min(m, k/2);
        int pb = k-pa;
